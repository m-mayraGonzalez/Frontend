...
export default {
  ...
  methods: {
    listar() {
      const me = this;
      axios
        .get(`usuario`, me.$store.getters.header)
    ...
    validar() {
      ...
      if (this.nombre.length < 1 || this.nombre.length > 50) {
        this.validaMensaje.push(
          "El nombre del Usuario debe tener entre 1-30 caracteres."
        );
      }
      if (this.descripcion.length > 255) {
        this.validaMensaje.push(
          "La descripción del Usuario no debe tener más de 255 caracteres."
        );
      }
    ...
    eliminar() {
      ...
      axios.delete(`usuario/${me._id}`, this.headers).then((datos) => {
        console.log(datos);
      });
    },
  },
